...
#    self.num_enemies = 0
#    self.enemy_type = 0
    self.enemy_type_counts = [3, 5, 6, 8]
    self.wall_type = {}
  ...
  def GetEnemyText(self):
    ...
    if 0 <= self.num_enemies and self.num_enemies <= 3:
      actual_num_enemies = self.enemy_type_counts[self.num_enemies]
    ...
    if self.enemy_type > 0:
      return "%s %s" % (actual_num_enemies,
                        zelda_constants.ENEMY_NAME[self.enemy_type])
    return "         "
"""
  ...
  def GetEnemyText(self):
    ...
    if self.has_mixed_enemies:
      ...
      return "Mix: %s %x" % (actual_num_enemies, self.enemy_type)
    return ("%s %s" %
            (actual_num_enemies, zelda_constants.ENEMY_NAME[self.enemy_type]))

  def InitializeFromRomData(self, rom_data):
  ...
  def CanMoveWithoutOpeningShutters(self, direction):
    return self.wall_type[direction] not in [1, 7]  # solid wall or shutter

  def CanMoveWithoutLadder(self, entry_direction, exit_direction):
  ...
  def CanDefeatEnemiesOrGetItemWithoutDoingSo(self, missing_item):
    if not self.is_drop_item:
      return True
    return self.CanDefeatEnemies(missing_item)

  def CanDefeatEnemiesOrBlockClipOrRightStairs(self, missing_item):
    if self.room_type in (zelda_constants.DIAMOND_ROOM_TYPE,
                          zelda_constants.RIGHT_STAIRS_ROOM_TYPE):
      return True
    return self.CanDefeatEnemies(missing_item)

  def CanDefeatEnemies(self, missing_item):
  ...
  def HasStairwayItem(self):
    return self.stairway_item > 0
  def GetStairwayItem(self):
    return self.stairway_item
  def SetStairwayItem(self, item_type):
    self.stairway_item = item_type

  def HasStairwayPassageRoom(self):
    return self.stairway_passage_room >= 0x00
  def GetStairwayPassageRoom(self):
    return self.stairway_passage_room
  def SetStairwayPassageRoom(self, other_room, stairway_letter):
    self.stairway_passage_room = other_room
    self.stairway_passage_num = stairway_letter

  def GetLeftExit(self):
    return self.left_exit
  def GetRightExit(self):
    return self.right_exit

  def SetLevelNumber(self, level_num):
  ...
  def WasAlreadyVisited(self):
    return self.already_visited
  def ClearVisitMark(self):
    self.already_visited = False

  def GetRoomType(self):
    return self.room_type
  def GetRoomTypeText(self):
  ...
  def GetItemText(self):
    ...
    if (self.item_type != 0x03 and
        self.item_type in zelda_constants.ITEMS.keys()):
      room_item_text = "%s%s" % (
          "D " if self.is_drop_item else "",
          zelda_constants.ITEMS[self.item_type])

    if self.HasStairwayPassageRoom() and room_item_text != "":
      return "S%s,%s" % (self.stairway_passage_num, room_item_text[0:6])
    if self.stairway_item >= 0 and room_item_text != "":
      return "%s,%s" % (
          zelda_constants.ITEMS[self.stairway_item][0:5], room_item_text[0:4])
    return "%s%s" % (stairway_text, room_item_text)

  def GetAsciiTest(self):
    string_parts = []
    string_parts.append(
        "-----%s%s-----" %
    ...
    string_parts.append(
        "%s%s%s" %
        (zelda_constants.WALL_TYPE_CHAR[self.wall_type[Direction.WEST]][1],
